...
import SejarahManagement from "@/components/admin/sejarah-management"
import TentangManagement from "@/components/admin/tentang-management"
...
export default async function AdminSejarahPage() {
  ...
  // Data umum desa (hero/about)
  const { data: villageInfo } = await supabase.from("village_info").select("*").single()
  // Ambil konten Sejarah terbaru yang dipublikasikan untuk ditampilkan seperti halaman publik
  const { data: sejarah } = await supabase
    .from("pages")
    ...
    .limit(1)
  const latestSejarah = sejarah && sejarah.length > 0 ? sejarah[0] : null
  ...
  return (
    <AdminLayout user={user} admin={admin}>
      <div className="space-y-6">
        {/* Header */}
        <section className="py-6 rounded-lg border bg-white">
          <div className="px-6">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Kelola Profile Desa</h1>
          </div>
        </section>

        {/* Dua kolom: kiri konten publik, kanan panel kelola */}
        <div className="grid grid-cols-1 gap-8 items-start">
          {/* Kiri: tampilan publik */}
          {false && (
          <div className="space-y-8">
            {/* Grid About seperti publik */}
            <section>
            ...
            {/* Sejarah + kontrol edit/delete */}
            <section>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-bold text-gray-900">Sejarah Desa</h3>
                {latestSejarah && <InlineActions pageId={latestSejarah.id} />}
              </div>
              {latestSejarah ? (
                <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: latestSejarah.content ?? "" }} />
              ) : (
          ...
          {/* Kanan: panel kelola sejajar */}
          <div className="space-y-8" id="kelola">
            {/* Kelola Tentang Desa (blok-blok) */}
            <TentangManagement currentUserId={user.id} />

            {/* Kelola Sejarah Desa: mengikuti desain Deskripsi & Sejarah */}
            <SejarahManagement currentUserId={user.id} />
          </div>
        </div>
      </div>
    </AdminLayout>
